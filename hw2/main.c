...
void record_operator(int salary) {
    emp_record_t e;
    e.emp_id = o;
    strcpy(e.emp_name,name);
    e.emp_salary = salary;
    operatorIdFile = fopen("operatorIdFile.bin", "ab");
    fwrite(&e, sizeof(e), 1, operatorIdFile);
    fclose(operatorIdFile);
}
...
int main() {
    ...
    if(o ==0) {
        int salary;
        printf("Please enter the operator ID you want to add:");
        scanf("%d",&id);
        printf("Please enter the operator NAME you want to add:");
        scanf("%s",name);
        printf("Please enter the operator SALARY you want to add:");
        scanf("%d",&salary);
        record_operator(salary);
